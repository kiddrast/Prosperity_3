...
class Trader:
    def __init__(self):
        ...
        self.CSI = 45  # Indice di luce solare critico empiricamente scoperto
    ...
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        for product in ['SQUID_INK', 'CROISSANTS', 'JAMS', 'DJEMBES']:
            order_depth = state.order_depths[product]
        ...
        for product in ['MAGNIFICENT_MACARONS']:
            ...
            # Retrieve sunlight and fundamental indicators
            ...
            if sunlight < self.CSI:
                ...
                if spread >= 2:
                    ...
                    print(f"[CSI MM BUY] {bid_volume}x @ {bid_price}, Sunlight={sunlight}")
                    print(f"[CSI MM SELL] {ask_volume}x @ {ask_price}, Sunlight={sunlight}")
                    orders.append(Order(product, bid_price, bid_volume))
                    orders.append(Order(product, ask_price, -ask_volume))
            else:
                if best_ask < fair_value:
                    ...
                    if volume > 0:
                        print(f"[FAIR VALUE BUY] {volume}x @ {best_ask} < FV({fair_value:.2f})")
                ...
                if best_bid > fair_value:
                    ...
                    if volume > 0:
                        print(f"[FAIR VALUE SELL] {volume}x @ {best_bid} > FV({fair_value:.2f})")
        ...
        for product in ['CROISSANTS', 'JAMS']:
            own_trades = state.own_trades[product][-1]
